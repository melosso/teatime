...
| `cover`   | A feature image. It accepts the same width attributes as inline images, so `cover: /assets/hero.webp {.full}` gives you a full width hero. Two height attributes tune how tall it sits: `{.short}` crops it to a slim band, `{.tall}` gives it more room, and they combine with a width as in `{.wide .short}`. A hex code like `cover: #3a5f4a` paints a flat colour block instead, no image needed |
