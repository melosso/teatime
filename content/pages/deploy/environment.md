...
It answers `503` with `"status": "empty"` when no content has been built, which is what an uptime monitor such as [Uptime Kuma](https://github.com/louislam/uptime-kuma) should watch for. The route adapts no rate limit, so polling it every few seconds is fine.
