...
internal static class BlogEndpoints
{
    ...
    {
        app.MapMethods("/", HttpVerbs.GetAndHead, (HttpContext ctx, PostService posts, BlogPageResponder responder, DocsOptions options, ContentService content) =>
            RenderHome(ctx, posts, responder, options, 1, content));

        app.MapMethods("/page/{n:int}", HttpVerbs.GetAndHead, (int n, HttpContext ctx, PostService posts, BlogPageResponder responder, DocsOptions options, ContentService content) =>
            RenderHome(ctx, posts, responder, options, n, content));

        app.MapMethods("/posts/{slug}", HttpVerbs.GetAndHead, RenderPost);
        app.MapMethods("/tags", HttpVerbs.GetAndHead, RenderTagIndex);
        app.MapMethods("/tags/{tag}", HttpVerbs.GetAndHead, (string tag, HttpContext ctx, PostService posts, BlogPageResponder responder, ContentService content, DocsOptions options) =>
            RenderTag(tag, ctx, posts, responder, content, options, 1));
        app.MapMethods("/tags/{tag}/page/{n:int}", HttpVerbs.GetAndHead, (string tag, int n, HttpContext ctx, PostService posts, BlogPageResponder responder, ContentService content, DocsOptions options) =>
    ...
    private static async Task RenderAuthorIndex(HttpContext ctx, AuthorService authors, BlogPageResponder responder, ContentService content)
    {
        ...
        if (TryRedirect(ctx, custom, basePath, content))
            return;
    ...
    private static async Task RenderArchive(HttpContext ctx, PostService posts, BlogPageResponder responder, ContentService content, AuthorService authorService)
    {
        ...
        var columns = custom?.ArchiveColumns;
        var html = RenderCustomIndex(custom, ArchiveRenderer.BuildYears(years, basePath, columns, authorService), route.Slug, basePath,
            () => ArchiveRenderer.Build(years, basePath, columns, authorService));
    ...
    private static bool TryRedirect(HttpContext ctx, Models.DocumentationPage? custom, string basePath, ContentService content)
    {
        if (custom?.Redirect is not { Length: > 0 } target)
            return false;

        if (!PageRequestHandler.TryResolveRedirect(target, basePath, ctx, content.SiteConfig, out var resolved))
            return false;

        ctx.Response.Redirect(resolved, permanent: false);
        return true;
    }

    // Custom index: cover, then prose with the list injected at {{name}}; falls back when no page.md exists.
