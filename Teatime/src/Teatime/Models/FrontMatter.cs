...
public sealed record FrontMatter
{
    ...
    /// <summary>Columns for a custom archive page's <c>{{archive}}</c> rows, in order: <c>date</c>, <c>title</c>, <c>author</c>, <c>authorImage</c>.</summary>
    public List<string>? ArchiveColumns { get; init; }
