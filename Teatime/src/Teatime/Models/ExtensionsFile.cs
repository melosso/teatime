...
/// <summary>
/// Beacon consent management for newsletter sign-up; the API key stays server side behind <c>/api/subscribe</c>.
/// </summary>
public sealed class BeaconOptions
{
    ...
    /// <summary>
    /// Leaves an existing consent record untouched, so re-submitting an address cannot undo an earlier opt-out.
    /// </summary>
    public bool SkipPermissionUpdate { get; set; } = true;
}

/// <summary>
/// Self-hosted Listmonk; sign-ups use the public subscription endpoint, so there is no credential to keep secret.
/// </summary>
...
/// <summary>
/// Mailchimp; its API key grants full account access, so <see cref="ApiKey"/> is only accepted as a <c>${ENV_VAR}</c> reference.
/// </summary>
