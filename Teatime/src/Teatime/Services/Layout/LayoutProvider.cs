...
public static partial class LayoutProvider
{
    public static string GetLayout(
        ...
        ITeatimeTheme? theme = null,
        ITeatimeStructure? structure = null)
    {
        ...
        // Empty when no newsletter back end is configured, so sites without one get no dead button.
        var subscribeFormHtml = NewsletterBlock.Render("{}");
        var subscribeTriggerHtml = subscribeFormHtml.Length > 0 ? BuildSubscribeTrigger() : "";
        var subscribeOverlayHtml = subscribeFormHtml.Length > 0 ? BuildSubscribeOverlay(subscribeFormHtml) : "";
        ...
        var activeStructure = structure ?? StructureRegistry.Default;
        var themeTokenCss = ThemeCssBuilder.BuildTokenCss(activeTheme, themeMode);
        // Structure CSS last, so a page shape choice always wins over a theme's own smaller touches.
        var componentCss = activeStructure.ComponentCss.Length > 0
            ? activeTheme.ComponentCss + "\n" + activeStructure.ComponentCss
            : activeTheme.ComponentCss;
...
<body>
    ...
    private static string BuildSubscribeTrigger()
    {
        var l = Localization.Current;
        return $@"<button type=""button"" class=""subscribe-trigger"" id=""subscribe-trigger"" data-subscribe
                    aria-haspopup=""dialog"" aria-controls=""subscribe-overlay"">{HtmlEncode(l.NewsletterHeading)}</button>";
    }

    private static string BuildSubscribeOverlay(string formHtml)
    {
        var l = Localization.Current;
        return $@"
    <div class=""subscribe-overlay"" id=""subscribe-overlay"" hidden>
        <div class=""subscribe-modal"" role=""dialog"" aria-modal=""true"" aria-label=""{HtmlEncode(l.NewsletterHeading)}"">
            <button type=""button"" class=""subscribe-modal-close icon-btn"" id=""subscribe-modal-close"" aria-label=""{HtmlEncode(l.NewsletterHeading)}"">
                <svg viewBox=""0 0 24 24"" fill=""none"" stroke=""currentColor"" stroke-width=""2"" stroke-linecap=""round"" aria-hidden=""true""><path d=""M18 6L6 18M6 6l12 12""/></svg>
            </button>
            {formHtml}
        </div>
