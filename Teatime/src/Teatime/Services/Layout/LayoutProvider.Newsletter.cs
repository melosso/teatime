...
{
    /// <summary>
    /// Styles and submit handler for the newsletter form, emitted only on pages that carry one; it posts to /api/subscribe, so connect-src stays 'self'.
    /// </summary>
    private static string GetNewsletterAssets(string basePath, string? nonce)
    {
        ...
        (function () {{
            ...
            document.querySelectorAll('form[data-newsletter]').forEach(function (form) {{
                ...
                // Closes the form for this page view only: one submit from a browser is not a subscription.
                function settle() {{
