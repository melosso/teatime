...
public static partial class LayoutProvider
{
    private static string GetScripts(bool enableLiveReload, bool enableDarkMode, long buildVersion, string basePath, string? nonce = null, bool staticSearch = false)
    {
        ...
        document.addEventListener('DOMContentLoaded', function() {{
            ...
            var subscribeTrigger = document.getElementById('subscribe-trigger');
            var subscribeOverlay = document.getElementById('subscribe-overlay');
            if (subscribeTrigger && subscribeOverlay) {{
                var subscribeModalClose = document.getElementById('subscribe-modal-close');
                var subscribeLastFocused = null;
                var openSubscribe = function() {{
                    subscribeLastFocused = document.activeElement;
                    subscribeOverlay.hidden = false;
                    requestAnimationFrame(function() {{ subscribeOverlay.classList.add('open'); }});
                    document.documentElement.style.overflow = 'hidden';
                    var email = subscribeOverlay.querySelector('input[name=""email""]');
                    (subscribeModalClose || email || subscribeOverlay).focus();
                }};
                var closeSubscribe = function() {{
                    subscribeOverlay.classList.remove('open');
                    subscribeOverlay.hidden = true;
                    document.documentElement.style.overflow = '';
                    if (subscribeLastFocused && subscribeLastFocused.focus) subscribeLastFocused.focus();
                }};
                subscribeTrigger.addEventListener('click', openSubscribe);
                if (subscribeModalClose) subscribeModalClose.addEventListener('click', closeSubscribe);
                subscribeOverlay.addEventListener('mousedown', function(e) {{ if (e.target === subscribeOverlay) closeSubscribe(); }});
                document.addEventListener('keydown', function(e) {{ if (!subscribeOverlay.hidden && e.key === 'Escape') closeSubscribe(); }});
            }}
                            ...
                            if (total === 0) {{
                                ...
                                if (authors.length) {{
                                    ...
                                    authors.forEach(function(a) {{
                                        ...
                                        html += '<a href=""{basePath}/' + escapeHtml(a.url) + '/"" class=""search-result-item search-hit-media"" role=""option"" id=""search-result-' + i + '"" aria-selected=""false"" tabindex=""-1"">' +
                                ...
                                if (tags.length) {{
                                    ...
                                    tags.forEach(function(t) {{
                                        html += '<a href=""{basePath}/' + escapeHtml(t.url) + '/"" class=""search-result-item search-hit-media"" role=""option"" id=""search-result-' + i + '"" aria-selected=""false"" tabindex=""-1"">' +
