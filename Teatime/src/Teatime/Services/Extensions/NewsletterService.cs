...
/// <summary>
/// Sends a sign-up to the configured back end; credentials and provider replies stay server side, as either can disclose subscriber state.
/// </summary>
public sealed class NewsletterService(HttpClient http, IExtensionSource extensions, ILogger<NewsletterService> logger)
{
    ...
    /// <summary>
    /// Providers use one status for a bad address and a bad setting, so only an explicit complaint about the email reaches the reader.
    /// </summary>
