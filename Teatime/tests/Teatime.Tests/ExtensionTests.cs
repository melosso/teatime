...
public sealed class SecurityHeadersExtraSourceTests
{
    ...
    /// <summary>Mermaid injects unnonced &lt;style&gt; elements, and a nonce in style-src would block them.</summary>
    [Fact]
    public void NonceStylesFalse_KeepsUnsafeInlineForStylesOnly()
    {
        var csp = SecurityHeaders.BuildNonceCsp(SecurityHeaders.DefaultCsp, "n0nce", nonceStyles: false);
        var directives = csp.Split(';').Select(d => d.Trim()).ToList();

        Assert.Contains(directives, d => d == "style-src 'self' 'unsafe-inline'");
        Assert.Contains(directives, d => d == "script-src 'self' 'nonce-n0nce'");
        Assert.DoesNotContain("style-src 'self' 'nonce-n0nce'", csp);
    }

    [Fact]
    public void NonceStylesDefault_NoncesStyleSrc()
    {
        var csp = SecurityHeaders.BuildNonceCsp(SecurityHeaders.DefaultCsp, "n0nce");

        Assert.Contains("style-src 'self' 'nonce-n0nce'", csp);
